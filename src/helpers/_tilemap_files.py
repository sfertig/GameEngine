...
def gen_collision_shapes(self, COL_FULL, COL_HALF_TOP, COL_HALF_BOTTOM, COL_HALF_LEFT, COL_HALF_RIGHT, SEARCH_TIME):
    ...
    for pos in half_top:
        if pos in used: continue
        #else creat new collision and run search
        col = CollisionRect(pos[0]*self.width, pos[1]*self.height, self.width, self.height//2, self.collisionLayers)
        #run search through tiles
        for i in range(SEARCH_TIME):
            for tile in full:
                if tile in used: continue
    ...
    #half bottom
    used.clear()
    for pos in half_bottom:
        if pos in used: continue
        #else creat new collision and run search
        col = CollisionRect(pos[0]*self.width, (pos[1]*self.height)+self.height//2, self.width, self.height//2, self.collisionLayers)
        #run search through tiles
        for i in range(SEARCH_TIME):
            for tile in full:
                if tile in used: continue
    ...
    for t in half_left:
        ...
        #else this is current shape to attempt and match
        #used.append(shape)
        shape = CollisionRect(t[0]*self.width, t[1]*self.height, self.width//2, self.height, self.collisionLayers)
        _tiles.append(shape)
        for i in range(SEARCH_TIME):
            for other_shape in half_left:
                if other_shape in used: continue
                #elif above, and width is the same, and x is the same
                if other_shape.y == shape.y-other_shape.height and other_shape.width == shape.width and other_shape.x == shape.x:
                    shape.y -= other_shape.height
    ...
    for t in half_right:
        ...
        #else this is current shape to attempt and match
        #used.append(shape)
        shape = CollisionRect((t[0]*self.width)+self.width//2, t[1]*self.height, self.width//2, self.height, self.collisionLayers)
        _tiles.append(shape)
        for i in range(SEARCH_TIME):
            for other_shape in half_right:
                if other_shape in used: continue
                #elif above, and width is the same, and x is the same
                if other_shape.y == shape.y-other_shape.height and other_shape.width == shape.width and other_shape.x == shape.x:
                    shape.y -= other_shape.height
